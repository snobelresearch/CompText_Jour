...
- Use the tidyverse package dplyr to sort and filter -- Identify trends
  in your data.
...
Business_filter <- MediaBucks %>%
  filter(grepl("?Business", COMPANY))
summary(Business_filter$Salary)

Bloomberg_filter <- MediaBucks %>%
  filter(grepl("?Bloomberg", COMPANY))
summary(Bloomberg_filter$Salary)

#Which one is higher?
business_filter_mean <- mean(Business_filter$Salary)
bloomberg_filter_mean <- mean(Bloomberg_filter$Salary)
if (business_filter_mean > bloomberg_filter_mean) {
  print("Business is higher")
}  else {
  print ("Bloomberg is higher")
}
```
...
Editors <- subset(MediaBucks, TITLE == "Editor")
