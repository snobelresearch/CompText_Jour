...
Crime_Cleaned_Count <- SF |> 
  count(crime_cleaned) |> 
  arrange(desc(n))

head(Crime_Cleaned_Count)
...
No_Action <- SF |> 
  filter(disposition1 %in% c("No_Dispo", "Gone")) |> 
  count(disposition1) |> 
  arrange(desc(n))
...
Serious_Action <- SF |> 
  filter(disposition1 %in% c ("Arrest", "Cited", "Criminal", "Medical")) |> 
  count(disposition1) |>
  arrange((desc(n)))
...
SF %>% 
  ...
  #Changes angle of x axis labels
  #coord_flip() +    #this makes it a horizontal bar chart instead of vertical
  labs(title = "Number of calls by year and month", 
       subtitle = "Calls Based on SF PD Service Call Data by Month 2017-2019",
       caption = "Graphic by Nobel",
       y="Number of Calls",
       x="Year-Month")
```
...
```{r}
#Clean all of the Crime Columns #In the example above, we
#cleaned just the crime2 column. #Now do the same with "crime3", "crime4"
#and join them together into a combined dataframe

#Column_3_Clean
SF <- SF %>%
   mutate(crime3_cleaned = case_when(
...
#Column_4_Clean
SF <- SF %>%
   mutate(crime4_cleaned = case_when(
...
Together <- SF |> 
  select(crime_cleaned, crime3_cleaned, crime4_cleaned)
